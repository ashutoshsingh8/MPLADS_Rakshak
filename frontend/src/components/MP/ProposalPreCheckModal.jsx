...
export default function ProposalPreCheckModal({ isOpen, onClose, onSubmitSuccess }) {
  ...
  const handleRunPreCheck = async () => {
    if (!title.trim() || !description.trim()) {
      setError('Please provide a project title and description for guideline verification.');
      return;
    }
    setError('');
    setAiChecking(true);
    try {
      ...
      setComplianceResult({
        ...
        recommendation: 'Eligible for formal submission to District Authority (Pune).',
      });
    } finally {
      setAiChecking(false);
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-fade-in select-none">
      <div className="relative w-full max-w-2xl bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto space-y-4 text-xs text-slate-700 flex-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              >
                ...
                <option value="COMMUNITY_CENTER">Community Centers & Halls</option>
                <option value="EDUCATION">Education & School Infra</option>
                <option value="HEALTH">Public Health & Sub-Centers</option>
                <option value="SANITATION">Sanitation & Public Toilets</option>
                <option value="SPORTS">Sports Facilities & Parks</option>
              </select>
            </div>
          </div>
          ...
          <div>
            <label className="block font-bold text-slate-700 mb-1">
              Public Utility Description & Beneficiary Justification <span className="text-red-500">*</span>
            </label>
            <textarea
              rows={3}
              value={description}
            ...
            <div className="p-4 rounded-xl border bg-emerald-50/80 border-emerald-300 space-y-2 animate-scale-up">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-emerald-900 font-bold">
                  ...
                  <span>Proposal Pre-Screening: ELIGIBLE FOR SUBMISSION</span>
                </div>
                <span className="text-[10px] font-mono font-bold bg-emerald-200 text-emerald-900 px-2 py-0.5 rounded">
                  {Math.round((complianceResult.confidence || 0.94) * 100)}% Confidence
                </span>
              </div>
